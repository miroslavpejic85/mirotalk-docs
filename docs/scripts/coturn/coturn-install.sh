...
if command -v nginx >/dev/null || command -v docker >/dev/null || command -v node >/dev/null; then
    warning "Some packages like nginx, docker, or nodejs are already installed. MiroTalk Coturn requires specific versions of these packages and will install them as part of its installation. Please start with a fresh Ubuntu install and run this script again."
fi
